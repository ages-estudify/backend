...
export class ExamMediaService {
  ...
  buildMediaKey(examId: string, extension: string): string {
    return `exams/${examId}/${randomUUID()}.${extension}`;
  }

  async uploadExamImage(examId: string, buffer: Buffer, mimeType: string): Promise<string> {
    ...
    const mediaKey = this.buildMediaKey(examId, extension);
