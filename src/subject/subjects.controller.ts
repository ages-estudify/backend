...
import { ApiBadRequestResponse, ApiNotFoundResponse, ApiOkResponse, ApiQuery, ApiResponse, ApiUnauthorizedResponse } from '@nestjs/swagger';
...
async pathsBySubject(@Param('id') id: string, @CurrentUser() user: JwtAuthUser): Promise<AllSubjectsPathsResponseDto> {
...
async countQuestions(@Param('topicId') pathId: string, @Query('type') type: 'ORIGINAL' | 'EXTERNAL', @CurrentUser() user: JwtAuthUser ): Promise<CountByPathAndTypeDto> {
