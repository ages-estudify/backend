...
describe('Subjects e2e flows', () => {
  ...
  beforeAll(async () => {
    ...
    const subject = await prisma.subject.create({
      data: {
        name: `e2e-subject-${randomUUID()}`,
        icon_url: 'https://example.com/subject-icon.png',
      },
    });
    ...
    const path = await prisma.path.create({
      data: {
        name: `e2e-path-${randomUUID()}`,
        text: 'Tópico para E2E subjects',
  ...
  beforeEach(async () => {
    ...
    await prisma.user.create({
      data: {
        id: createdUserId,
        full_name: `e2e subjects user ${randomUUID()}`,
        email: `e2e-subjects-${randomUUID()}@example.com`,
  ...
  it('lists subjects with total and answered question counts', async () => {
    ...
    const response = await request(app.getHttpServer()).get(baseSubjects).expect(200);
  ...
  it('returns paths for a subject with available and answered counts', async () => {
    await prisma.answer.create({
      data: {
        answer_date: new Date(),
        user_id: createdUserId,
  ...
  it('counts questions by path and type', async () => {
  ...
  it('returns 400 when count questions type is invalid', async () => {
